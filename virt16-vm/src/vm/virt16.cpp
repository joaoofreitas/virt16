...
namespace Virt16 {
    ...
    void virt16::step() {
        ...
        const unsigned int instr =
            (static_cast<unsigned int>(getMemory(pc)) << 16) |
             static_cast<unsigned int>(getMemory(pc + 1));
        ...
        switch (opcode) {
            ...
            case AND:
                X = FIELD_X(instr); Y = FIELD_Y(instr); Z = FIELD_Z(instr);
            ...
            case NOT:
                X = FIELD_X(instr); Y = FIELD_Y(instr);
                setRegister(X, ~getRegister(Y));
                break;
            ...
            case JZ:
                addr = FIELD_IMM(instr);
                if (getFlag(Flags::Z)) pc = addr;
                break;

            case JE:
                addr = FIELD_IMM(instr);
                if (getFlag(Flags::E)) pc = addr;
                break;
            ...
            case JG:
                addr = FIELD_IMM(instr);
                if (getFlag(Flags::G)) pc = addr;
                break;

            case JL:
                addr = FIELD_IMM(instr);
                if (getFlag(Flags::L)) pc = addr;
                break;
