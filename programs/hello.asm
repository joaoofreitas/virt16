; hello.asm — displays "Hello World!" on the 16x16 console
;
; Console character buffer starts at 0x2900 (16x16 = 256 words).
; Font data starts at 0x3100 (4 words per glyph, ASCII 32-127).
; Each console cell stores one ASCII code as a 16-bit word.
;
; Load this .bin in the VM, then switch to Console mode in the Monitor tab.

; Font data: 8x8 glyphs for ASCII 32 (space) through 127.
; Each glyph is 4 words: each word packs two rows of 8 pixels (high byte = top row).
.PLACE 0x3100 [
...
; Console character buffer: "Hello World!" starting at row 0, col 0.
; Each word is one ASCII character code.
.PLACE 0x2900 "Hello World!"

.main:
    LOAD SP, #0x4000    ; init stack
    LOAD DISP, #0x3000  ; set display base address
    HLT
